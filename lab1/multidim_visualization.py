...
class HighDimVisualizer:
    ...
    def plot_pca_2d_3d(self):
        ...
        axes[0].legend(handles, list(self.target_class_names.values()), #[f'Class {int(l.split("{")[1].split("}")[0])}' for l in labels]
                      title="Классы")
        ...
        ax3d.legend(handles3d, list(self.target_class_names.values()), #[f'Class {int(l.split("{")[1].split("}")[0])}' for l in labels]
                      title="Классы")
    ...
    def plot_tsne_visualization(self):
        ...
        for i, perplexity in enumerate(perplexities):
            ...
            #if i == 0:
            handles, labels = scatter.legend_elements()
            axes[i].legend(handles[:5], list(self.target_class_names.values()), title="Классы", fontsize=8) #[f'Class {int(l.split("{")[1].split("}")[0])}' for l in labels[:5]]
        ...
        try:
            ...
            for n_neighbors in n_neighbors_list:
                for min_dist in min_dists:
                    ...
                    handles, labels = scatter.legend_elements()
                    axes[plot_idx].legend(handles, list(self.target_class_names.values()), #[f'Class {int(l.split("{")[1].split("}")[0])}' for l in labels]
                                title="Классы")
    ...
    def plot_parallel_coordinates(self, n_features=10):
        ...
        legend_elements = [Line2D([0], [0], color=colors_plt[i], lw=2, 
                                 label=list(self.target_class_names.values())[i]) #f'Class {int(class_val)}'
                          for i, class_val in enumerate(unique_classes)]
    ...
    def plot_radar_chart(self, n_features=6):
        ...
        for idx, class_val in enumerate(unique_classes):
            ...
            # Рисуем радар-чарт
            axes[idx].plot(angles, values, 'o-', linewidth=2, label=list(self.target_class_names.values())[idx]) #f'Class {int(class_val)}'
    ...
    def compare_dim_reduction_methods(self):
        ...
        for idx, (name, model) in enumerate(methods):
            ...
            try:
                ...
                handles, labels = scatter.legend_elements()
                axes[idx].legend(handles, list(self.target_class_names.values()), #[f'Class {int(l.split("{")[1].split("}")[0])}' for l in labels]
                            title="Классы")
